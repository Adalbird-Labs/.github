...
| Adalbird Physical Device Lab | Cross-product physical Android regression broker and evidence path, centered on `platform-automation`; S10 is primary persistent capacity and S25 Ultra is opportunistic compatibility capacity | **Partially.** Direct Chat can manage broker/config/code and inspect GitHub evidence. Historical S10 runner/ADB diagnostics are proven, but **live broker-mediated product execution is currently disabled** until the fresh hardened central-runner proof (ADL-392) and least-privilege cross-repo artifact credential (ADL-464) are complete. S25 still needs one-time local bootstrap before use. | **No** for normal broker operations once the gates are cleared. One-time founder/local setup is still required for runner availability/service mode, Chrome Remote Desktop emergency admin, GitHub App credential, and per-device ADB/RSA onboarding. | `platform-automation` is the central shared-resource source of truth. Durable queue, priority/FIFO selection, exclusive leases, runtime/heartbeat state, reservation transaction, real artifact producer allowlist, artifact provenance and token-safe cross-repo observation are implemented. Static registry presence never proves a phone is online: no fresh runtime state/lease/provenance => no device mutation. Product repos must not bypass the broker or address raw serials. S10 is the only baseline capacity; S25 never counts toward standing SLA/capacity. Keep execution sequential unless measured evidence justifies safe parallelism. Completion callbacks stay privacy-safe (PASS/FAIL, action, alias, run ID, commit SHA only). No paid device farm/hardware/cloud capacity without Wojtek approval. |
