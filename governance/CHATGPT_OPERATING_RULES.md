...
| Adalbird Physical Device Lab | Cross-product physical Android regression broker and evidence path, centered on `platform-automation`; S10 is primary persistent capacity and S25 Ultra is opportunistic compatibility capacity | **Yes for broker-mediated live execution on proven allowlisted flows.** Direct Chat can manage broker/config/code and inspect GitHub evidence. The hardened central S10 proof (ADL-392), least-privilege cross-repo GitHub App credential + metadata-only provenance proof (ADL-464), and central artifact-download/digest-verification/allowlisted-device executor proof (ADL-497) are PASS. S25 still needs one-time local bootstrap before use. | **No** for normal broker operations. One-time founder/local setup is still required for runner availability/service mode, Chrome Remote Desktop emergency admin and per-device ADB/RSA onboarding; the GitHub App credential setup for Grzybołaz/Dymistrz is complete. | `platform-automation` is the central shared-resource source of truth. Durable queue, priority/FIFO selection, exclusive leases, runtime/heartbeat state, reservation transaction, real artifact producer allowlist, artifact provenance, token-safe cross-repo observation, verified artifact download/digest checks and the allowlisted physical executor are implemented. Shared physical execution must go through the broker; product repos must not bypass it or address raw serials. Artifact-byte download/install/device mutation remains fail-closed unless the request is valid, runtime state is fresh, provenance/digest checks pass, an exclusive lease is active and the product/suite adapter is allowlisted. Static registry presence never proves a phone is online. S10 is the only baseline capacity; S25 never counts toward standing SLA/capacity. Keep execution sequential unless measured evidence justifies safe parallelism. Completion callbacks stay privacy-safe (PASS/FAIL, action, alias, run ID, commit SHA only). No paid device farm/hardware/cloud capacity without Wojtek approval. |
...
| Google Drive | Shared company and personal files/artifacts where used | **Yes.** As of 2026-09-21 both `contact@adalbirdlabs.com` (company) and `wmisiarz@gmail.com` (personal) are connected as separate Drive identities | No for supported connector actions | **Always select the intended linked Drive identity before reading or writing.** Adalbird Labs company operations default to `contact@adalbirdlabs.com`; personal Drive must not be used for company storage merely for convenience. Company shared artifacts should prefer the `Adalbird Labs` Shared Drive when appropriate. |
...
- Set the company Drive default to `contact@adalbirdlabs.com` and recorded the `Adalbird Labs` Shared Drive as the preferred home for company shared artifacts.


### v1.27 — 2026-09-21
- Marked ADL-497 central broker-mediated physical execution proof PASS after an end-to-end S10 install/launch/force-stop run through the trusted Windows Service route.
- Enabled live Physical Device Lab execution for proven allowlisted broker flows while preserving fail-closed request, runtime, provenance, digest, lease and adapter gates.
- Kept S10 as baseline capacity, S25 as opportunistic compatibility capacity, sequential execution by default and the founder-only Spend Golden Rule unchanged.
